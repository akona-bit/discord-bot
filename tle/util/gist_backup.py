...
class GistBackup:
    ...
    async def download(db_path: Path) -> bool:
        """Download database from Gist and write it to db_path. Returns True if successful."""
        if not constants.GIST_TOKEN or not constants.GIST_ID:
            logger.warning("GIST_TOKEN or GIST_ID not provided. Skipping backup download.")
            return False
                        ...
                        if "user.db" in files:
                            ...
                            if content:
                                ...
                                with open(db_path, "wb") as f:
                                    f.write(decoded)
                                logger.info("Successfully downloaded database from Gist.")
                                return True
                    else:
                        logger.warning(f"Failed to fetch Gist: {resp.status} - {await resp.text()}")
    ...
    async def upload(db_path: Path) -> bool:
        ...
        if not constants.GIST_TOKEN or not constants.GIST_ID:
            logger.warning("GIST_TOKEN or GIST_ID not provided. Skipping backup upload.")
            return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.patch(url, headers=headers, json=payload) as resp:
                    if resp.status == 200:
                        ...
                        logger.warning(f"Failed to update Gist: {resp.status} - {await resp.text()}")
        except Exception as e:
